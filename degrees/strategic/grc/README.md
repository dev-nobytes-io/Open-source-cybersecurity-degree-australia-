...
```mermaid
graph LR
    GRC_MAJ["GRC Major"] --> R1["GRC Analyst"]
    GRC_MAJ --> R2["Risk Manager"]
    GRC_MAJ --> R3["Compliance Manager"]
    GRC_MAJ --> R4["Security Auditor"]
    GRC_MAJ --> R5["IRAP Assessor (pathway)"]
```
...
```mermaid
graph TD
    AU_REG["Australian Regulatory<br/>Landscape for GRC"]

    AU_REG --> PRIV["Privacy Act 1988<br/><i>APPs, NDB scheme,<br/>OAIC oversight</i>"]
    AU_REG --> SOCI["Security of Critical<br/>Infrastructure Act 2018<br/><i>CIRMP, RACI, ASD powers</i>"]
    AU_REG --> APRA["APRA CPS 234<br/><i>Financial sector<br/>Information security standard</i>"]
    AU_REG --> ISM["Australian Government<br/>Information Security Manual (ISM)<br/><i>Government entities</i>"]
    AU_REG --> E8["ASD Essential Eight<br/><i>Baseline control framework</i>"]
    AU_REG --> IRAP["IRAP Process<br/><i>Infosec Registered Assessors Program<br/>Government cloud assessment</i>"]
    AU_REG --> ASX["ASX Corporate Governance<br/>Principles — Cyber Disclosure<br/><i>Listed companies</i>"]
```
