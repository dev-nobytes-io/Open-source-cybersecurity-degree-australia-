...
def main() -> int:
    ...
    intro = [
        "# Program Builder",
        "",
        "Tick the courses you want — from a designed-degree preset, one or more "
        "majors, or any individual mix — and the **NICE/DCWF work-role completion** "
        "bars and the four **KSAT coverage** radar (\"spider web\") charts update "
        "live. Everything runs in your browser; nothing to install or rebuild.",
        "",
        "> KSAT IDs are project-local (provisional) pending Framework Custodian "
        "mapping to official NICE/DCWF identifiers. The dataset embedded in this "
        "page is refreshed by `.github/scripts/program_builder.py` whenever unit "
        "content changes. Underlying coverage detail: [`../ksat-coverage.md`](../ksat-coverage.md).",
        "",
    ]
    page = "\n".join(intro) + STYLE + "\n" + CONTAINER + "\n"
